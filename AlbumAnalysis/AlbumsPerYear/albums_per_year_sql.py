...
df.createOrReplaceTempView("AlbumsPerYear")

albums_per_year = spark.sql("""
                            SELECT year_of_release, COUNT(DISTINCT artist_name, album_name) AS number_of_albums
                            FROM AlbumsPerYear
                            GROUP BY year_of_release
                            ORDER BY year_of_release
                            """)

albums_per_year.show()
                    
albums_per_year.\
         write\
        .format("json")\
        .mode("overwrite")\
        .save("file:///home/hadoop/Proyecto-Spark/AlbumAnalysis/AlbumsPerYear/output_sql.json")
